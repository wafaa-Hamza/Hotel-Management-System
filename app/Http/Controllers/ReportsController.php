...
use App\Models\TransactionTaxes;
...
use App\Http\Resources\ChartAccountBalanceResource;
...
class ReportsController extends Controller
{
    ...
    {
        //        $this->authorize('report-totPaymentOfCashierCloser');


        if ($request->user_id == null) {

            $Total_Closer_Payment = Transaction::with('created_by', 'payment_type', 'room', 'guest.profile')
    ...
    {
        //$this->authorize('report-totpaymentofcashiersummary');

        if ($request->user_id == null) {

            $Total_Summary_Payment = User::
            with(['cashiers'=>function($q)use($request){
                $q->where('hotel_date','>=',$request->Date_From)->where('hotel_date','<=',$request->Date_To);
            }])
            ->with(['transactions' => function ($q) use ($request) {
                $q->whereNot('payment_type_id', null)
                    ->whereDate('hotel_date', '>=', $request->Date_From)
                    ->whereDate('hotel_date', '<=', $request->Date_To)
                    ->select('created_by', 'payment_type_id', DB::raw('SUM(payment_amount) as total_payment_amount'))
                    ->groupBy('created_by', 'payment_type_id')->with('payment_type');
            }])
                ->whereHas('transactions', function ($qu) use ($request) {
                    $qu->whereNot('payment_type_id', null)
                        ->whereDate('hotel_date', '>=', $request->Date_From)
                        ->whereDate('hotel_date', '<=', $request->Date_To);
                })->get();
        } else {
    ...
    {
        //        $this->authorize('report-productivitybycompany');

        $ProductivityByCompany = Guests::with('company')
            ->whereDate('transactions.hotel_date', '>=', $request->start_date)
            ->whereDate('transactions.hotel_date', '<=', $request->end_date)
    ...
    public function ProductivityByCountry(Request $request)
    {
        ...
        $this->authorize('report-productivitybycountry');
    ...
    {
        //        $this->authorize('report-productivitybysourcebusiness');
    ...
    {
        //        $this->authorize('report-productivitybyMarketsegments');
    ...
    {
        //        $this->authorize('report-guestledgerreport');

        $Guest_ledger_charges = LedgerCat::with(['ledgers' => function ($q) use ($request) {
            $q->withSum(['transactions' => function ($query) use ($request) {
                $query->where('is_void', 0);
                $query->whereDate('hotel_date', $request->date);
                $query->groupBy('ledger_id');
                //  $query->withSum('taxes', 'amount');
            }], 'charge_amount');
            $q->whereHas('transactions', function ($query) use ($request) {
                $query->where('is_void', 0);
                $query->whereDate('hotel_date', $request->date);
                $query->groupBy('ledger_id');
                //  $query->withSum('taxes', 'amount');
            ...
            ->withSum(['transactionsLedCat' => function ($qu) use ($request) {
                $qu->where('is_void', 0);
                $qu->whereDate('hotel_date', $request->date);

                $qu->groupBy('ledger_cat_id');
            }], 'charge_amount')
            ->whereHas('transactionsLedCat', function ($qu) use ($request) {
        ...
        $Guest_ledger_payments = PaymentMode::with(['payment_types' => function ($q) use ($request) {
            $q->withSum(['transactions' => function ($query) use ($request) {
                $query->where('is_void', 0);
                $query->whereDate('hotel_date', $request->date);
            ...
            ->withSum(['transactions' => function ($que) use ($request) {
        ...
        $Guest_ledger_Taxes = Tax::withSum(['transactions_taxes' => function ($q1) use ($request) {
            $q1->whereHas('transaction', function ($q) use ($request) {
                $q->whereDate('hotel_date', $request->date)->where('is_void', 0);
            });
        }], 'amount')->whereHas('transactions_taxes', function ($q1) use ($request) {
            $q1->whereHas('transaction', function ($q) use ($request) {
                $q->whereDate('hotel_date', $request->date)->where('is_void', 0);
            });
        })->get();
        ...
        //balances
        $lastDayGuestLedger = DayCloseRecord::whereDate('hotel_date', Carbon::parse($request->date)->subDay())->first();
        $todayBalance = Transaction::whereDate('hotel_date', $request->date)->where('is_void', 0)->sum(DB::raw('charge_amount - payment_amount'));
        $preBalance = 0;

        if ($lastDayGuestLedger != null) {
            $preBalance = $lastDayGuestLedger->guest_ledger;
        } else {
            $preBalance = 0;
        };
        // dd('out');
        $CurrentGuestLedger = (is_null($preBalance) ? 0 : $preBalance) - (is_null($todayBalance) ? 0 : $todayBalance);
        //City Ledger Payment
        $paymentTypeID = Payment_type::whereHas('modes', function ($q) {
            $q->where('name', 'cityledger');
        })->select('id')->first();
        if ($paymentTypeID) {
            $CityLedgerPayment = Transaction::where('hotel_date', $request->date)->where('is_void', 0)->where('payment_type_id', $paymentTypeID->id)->sum('payment_amount');
        } else {
            $CityLedgerPayment = 0;
        }

        //Company Balance from Statment
        $companyBalance = Statement::sum(DB::raw('debit_amount - credit_amount'));

        // Reservation payments

        $reservationPayments = Transaction::where('hotel_date', $request->date)->where('is_void', 0)
            ->whereHas('guest', function ($q) {
                $q->where('is_reservation', 1);
        ...
        $return_amounts = [
            ...
            'Guest_ledger_taxes' => $Guest_ledger_Taxes,
            'pre_balance' => $preBalance,
            'today_balance' =>  $todayBalance,
            'current_guest_ledger' => $CurrentGuestLedger,
            'city_ledger_payment' => $CityLedgerPayment,
            'company_balance' => $companyBalance,
            'reservation_balance' => $reservationPayments
    ...
    {
        //        $this->authorize('report-voidedtransactions');

        $voided_transaction = Transaction::with(['guest.profile', 'created_by', 'room'])
            ->where('is_void', 1)
    ...
    public function Transactions_Details(Request $request)
    {
        ...
        $transactions_details = Transaction::with(['guest.profile', 'created_by', 'room'])
            ->where('is_void', 0)
            ->where('hotel_date', '>=', $request->start_date)
            ->where('hotel_date', '<=', $request->end_date)
    ...
    {
        //        $this->authorize('report-transactionsdetailsbyuser');
    ...
    {
        //        $this->authorize('report-inhouseguestsreport');
    ...
    public function Room_Change_Report(Request $request)
    {
        ...
        $this->authorize('report-roomchangereport');


        $room_change_report = Room_change_history::with('user', 'RoomChangeFrom', 'RoomChangeTo')
            ->WhereDate('hotel_date', '>=', $request->startDate)
    ...
    {
        //        $this->authorize('report-ratechangereport');

        $rate_change_report = Ratechange::with('users', 'room', 'RateCodeFrom', 'RateCodeTo')
    ...
    {
        //        $this->authorize('report-actualcheckedinandarrivalsreport');
    ...
    {
        //        $this->authorize('report-checkedoutanddeparturereport');

        $Checked_Out_And_Departure_Report = Guests::with('profile', 'room', 'checkout_by', 'window')
    ...
    {
        //        $this->authorize('report-revenuebydatesbyledgercat');
    ...
    {
        //        $this->authorize('report-receiptvoucherlistreport');
    ...
    {
        //        $this->authorize('report-paymentvoucherlistreport');
    ...
    {
        //        $this->authorize('report-roomsstatusreport');
    ...
    {
        //        $this->authorize('report-ooandosreport');
    ...
    {
        //        $this->authorize('report-reservationreport');

        $ret_reser = [];
        $ret_reser2 = [];
        $res_status = [];
        $res_status = $request->res_status;
        $deposit = Transaction::where('charge_amount', null)->sum('payment_amount');

        $reservation_overview = Guests::with(['profile', 'company', 'reservation_status'])->withCount('children')
            ->whereBetween('in_date', [$request->startDate, $request->endDate])
            ...
            ->get()->map(function ($qu) use ($deposit) {
                $qu->deposit = $deposit;
                return $qu;
            });


        foreach ($reservation_overview  as $Reservation) {
            $ret_reser = [];
            $guest_id = ['guest_id' => $Reservation->id];
            $Balance = $this->generalInterface->guestBalance($guest_id);



            $ret_reser =  ['Balance' => $Balance, 'res' => $Reservation];
            array_push($ret_reser2, $ret_reser);
        }

        return response()->json(['reservation_overview' => $ret_reser2]);
    }
    public function Res_Amount(Request $request)
    {
        //        $this->authorize('report-resamount');
    ...
    {
        //        $this->authorize('report-incomestatementreport');
    ...
    {
        //        $this->authorize('report-incomestatementreportmtdytd');
    ...
    {
        //        $this->authorize('report-highbalancerepor');
        ...
        foreach ($high_balance as $oneGuest) {
            ...
            if ($Balance > 0) {
                ...
                array_push($ret_guest, ['Balance' =>  $Balance]);

                array_push($data_With_Balance, $ret_guest);
            }
        }


        return response()->json(['Guests_With_Balance' => $data_With_Balance]);
    }

    public function Occupancy_By_Month(Request $request)
    {
        //        $this->authorize('report-occupancybymonth');
    ...
    {
        //        $this->authorize('report-occupancybymtd');

        $Occupancy_ByMonthTD = DayCloseRecord::whereBetween(DB::raw('DATE(hotel_date)'), [Carbon::parse($request->hotel_date)->startOfMonth(), $request->hotel_date])
            ->orderBy('hotel_date')->selectRaw('*, (total_rate_room /oc_rooms) as ARR, 
            total_fb + total_others as  OSales,
            ( oc_rooms/total_room)*100 as OCCPercentage')
            ->get();
    ...
    {
        //        $this->authorize('report-housekeepingbystatus');
    ...
    {
        //        $this->authorize('report-dailysalesreport');
        ...
        foreach ($daily_sales as $daily_sale) {
           ...
           (oc_rooms/total_room )*100 as occupancyPer')
                ->orderBy('hotel_date', 'asc')
                ->get()
    ...
    {
        //        $this->authorize('report-reservationbycompany');
    ...
    {
        //        $this->authorize('report-voucherlist');
    ...
    {
        //        $this->authorize('report-transactionslistview');

        $arr = [];
        $transaction_list_view = Transaction::with('room', 'ledger', 'guest', 'payment_type')
            ->where(function ($q) use ($request) {
                ...
                if ($request->has('folio_no')) {
                    if (!empty($request->input('folio_no'))) {
                        $q->where('guest_id', $request->input('folio_no'));
                    }
                }

                if ($request->has('voucher_no')) {
                    if (!empty($request->input('voucher_no'))) {
                        $q->where('trans_no', $request->input('voucher_no'));
                    }
                }
                ...
                if ($request->has('room_no')) {
                    $roomArr = [];
                    $roomArr = $request->input('room_no');
                    if (!empty($roomArr)) {
                        $q->whereIn('room_id', $roomArr);
                    }
    ...
    {
        //        $this->authorize('report-manager1report');
    ...
    {
        //        $this->authorize('report-manager5report');
        ...
        $print_invoice = Guests::selectRaw('id ,in_date,out_date,no_of_nights,room_id,rm_rate ,no_of_pax,vat_no,checkout_at,created_inhousGuest_at,profile_id,company_id,room_type_id')
            ...
            ->with(['window' => function ($q) use ($request) {
                $q->with(['transactions' => function ($q1) {
                   // $q1->selectRaw('hotel_date,room_id,description,charge_amount,payment_amount,trans_no,transactions.created_at');
                    $q1->with('room');
                    $q1->with('taxes');
                    $q1->sum('Charge_amount');
                    $q1->with(['taxes'=>function($q){
                        $q->where('taxes.name','vat');                       
                    }]);
                    $q1->with(['taxes'=>function($q){
                    $q->with(['transactions_taxes' => function($q) {
    $q->join('taxes', 'transactions_taxes.tax_id', '=', 'taxes.id')
        ->selectRaw('transactions_taxes.tax_id, taxes.name, SUM(transactions_taxes.amount) as total_amount')
        ->groupBy('transactions_taxes.tax_id', 'taxes.name');
}]);
            ...
            ->get();
            // $print_invoice->first()->window->first()->transactions->map(function ($q) {
            //     $q['total_amount'] = 0;
            //      $q->taxes->map(function ($q1) use($q){
            //       $q['total_amount'] = $q1->pivot->sum('amount');
            //        // return $q1;
            //     });
            //     return $q;
            // });
            //test
            $print_invoice->first()->window->map(function ($q) {
                // $q['tax_name'] = 0;
                // $q['total_tax_amount'] = 0;
                $q->transactions->map(function($q1) use($q){
                    $q1['total_amount'] = 0;
                    $q1->taxes->map(function ($q2) use($q1,$q){
                     $q1['total_amount'] = $q2->pivot->sum('amount');
                     $q2->transactions_taxes->map(function($q3) use($q){
                        $q['tax_name_'.$q3->name] = $q3->name;
                        $q['total_tax_amount_'.$q3->name] = $q3->total_amount;
                     });
                   // return $q1;
                });
                });
           ...
           $InvoiceTaxAmount =($print_invoice->first()->window->first()->transactions) ? $print_invoice->first()->window->first()
           ->transactions->first()->total_amount : 0;
           $InvoiceTotalAmount =($print_invoice->first()->window->first()) ? $print_invoice->first()->window->first()
           ->transactions_sum_charge_amount : 0;
          //  return $print_invoice;

            //start new qr
            $saller = Setting::select('name','vat_no')->first();
            $invoice = Invoice::where('guest_id',$request->guest_id)
            ->select('created_at')
            ->latest()->first();
            $invoiceDataeZuluForamt =($invoice) ?  $invoice->created_at->format('Y-m-d\TH:i:s\Z') : 0;
            $generatedString = GenerateQrCode::fromArray([
    new Seller($saller->name), // seller name from setting
    new TaxNumber($saller->vat_no), // seller tax number from setting
    new InvoiceDate($invoiceDataeZuluForamt), // checkout_at from invoice date as Zulu ISO8601 @see https://en.wikipedia.org/wiki/ISO_8601
    new InvoiceTotalAmount($InvoiceTotalAmount), // invoice total amount from invoice SUM (Charge_amount in transaction in window)
    new InvoiceTaxAmount($InvoiceTaxAmount) // invoice tax amount from invoice VAT TAX sum amount
    ...
    public function Meal_Plan(Request $request)
    {
        ...
        $Guest_InHouse_with_MealPlan = Guests::with('profile', 'room', 'meal_package.meal', 'meal')

            ->whereDate('in_date', ' <=', $request->startDate)
            ->orWhereDate('out_date', '>=', $request->startDate)
            ->where(function ($q) {
                $q->where('meal_id', '!=', null);
                $q->orWhere('meal_package_id', '!=', null);
                return $q;
            })
            ->get();
        //return $Guest_InHouse_with_MealPlan;
    ...
    public function Meals($guest)
    {
        ...
        if (!empty($guest->meal->meal_type)) {
            ...
            } elseif ($guest->meal->meal_type == 'SU') {
        ...
        } elseif (!empty($guest->meal_package->meal)) {
    ...
    private function sales_in_future($dateLoop, $cat_id)
    {
        ...
        //        $this->authorize('report-salesinfuture');
    ...
    {
        //        $this->authorize('report-historyandforecast');
        ...
        for ($formDate = $start; $formDate->lte($end); $formDate->addDay()) {
            ...
            if ($formDate < $currentDate) {
                ...
                $totRoom = DayCloseRecord::whereDate('hotel_date', '=', $formDate)
                    ->sum('total_room');
                if ($totRoom == 0) {
                    $totRoom = Room::where('rm_max_pax', '>', 0)->count();
                    $RateRoom = 0;
                } else {
    ...
    public function Monthwise_Revenue(Request $request)
    {
        ...
        //        $this->authorize('report-monthwiserevenue');
    ...
    {
        //        $this->authorize('report-monthwiseoccupancy');
    ...
    {
        //        $this->authorize('report-daywiseoccupancy');
    ...
    {
        //        $this->authorize('report-liveoccupancycalculations');
        ...
        $occupied_rooms = Room::where('rm_max_pax', '>', 0)->where('rm_active', 1)->where('fo_status', 'OC')->count();
        $vacant_rooms = Room::where('rm_max_pax', '>', 0)->where('rm_active', 1)->where('fo_status', 'VA')->count();
        ...
        $avg_room_rate = (($tot_rm_rate != 0) ? $tot_rm_rate : 1) / (($occupied_rooms != 0) ? $occupied_rooms : 1);
        ...
        $checkedOut_rooms = Guests::whereDate('out_date', $request->hotel_date)
            ->where('checked_out', 1)->where('is_dummy', 0)->count();

        $expected_departures = Guests::whereDate('out_date', $request->hotel_date)
            ->where('is_checked_in', 1)->where('checked_out', 0)->where('is_dummy', 0)->count();


        $expected_occupied_tonight = Guests::whereDate('in_date', '<=', $request->hotel_date)
            ->whereDate('out_date', '>=', $request->hotel_date)

            ->where('is_cancel', 0)->where('is_dummy', 0)->count();
    ...
    {
        //        $this->authorize('report-findfolio');
    ...
    {
        $voucherPrinting = Transaction::with(['payment_type', 'created_by', 'guest.profile', 'room'])

            ->where('id', $request->transaction_id)
            ->get();

        return response()->json(['voucherPrinting' => $voucherPrinting]);
    }
    ...
    public function GetUpdatedReservation(Request $request)
    {

        $updated_resrvation = Guests::with(['profile', 'company', 'createdBy', 'room'])
            ->with(['LogActivity' => function ($q) use ($request) {
                $q->whereBetween('created_at', [$request->startDate, $request->endDate]);
            }])
            ->where('updated_at', '!=', null)
            ->whereBetween('updated_at', [$request->startDate, $request->endDate])->get();

        return $updated_resrvation;
    }


    public function Transaction_Taxes(Request $request)
    {
        $transaction_taxes = Transaction::with(['guest.profile', 'created_by', 'room'])
            ->whereBetween('hotel_date', [$request->startDate, $request->endDate])

            ->with(['taxes' => function ($q) use ($request) {
        ...
        return response()->json(['Transaction_Taxes' => $transaction_taxes]);
    }
    ////////////////////////// not report///////////////////////////////////////////
    ...
    public function Balance($startDate, $endDate, $account_code)
    {

        $ids = ChartOfAccount::where('account_code', 'like',  $account_code . '%')->get(['id']);

        $profits_debit = JournalVoucherMaster::join('journal_voucher_details', 'journal_voucher_details.jv_master_id', '=', 'journal_voucher_masters.id')
        ...
        $profits_credit = JournalVoucherMaster::join('journal_voucher_details', 'journal_voucher_details.jv_master_id', '=', 'journal_voucher_masters.id')
            ->whereBetween('jv_date', [$startDate, $endDate])
            ->whereIn('journal_voucher_details.account_id',  $ids)
            ->sum('journal_voucher_details.credit');


        $Balance = $profits_debit - $profits_credit;
        return $Balance;
    }
    public function TotalCredit($startDate, $endDate, $account_code)
    {
        $ids = ChartOfAccount::where('account_code', 'like',  $account_code . '%')->get(['id']);
        $TotalCredit = DB::table('journal_voucher_masters')
    ...
    public function TotalDebit($startDate, $endDate, $account_code)
    {

        $ids = ChartOfAccount::where('account_code', 'like', $account_code . '%')->get(['id']);
        $TotalDepit = DB::table('journal_voucher_masters')
            ->join('journal_voucher_details', 'journal_voucher_details.jv_master_id', '=', 'journal_voucher_masters.id')
            ->whereBetween('journal_voucher_masters.jv_date', [$startDate, $endDate])
            ->whereIn('journal_voucher_details.account_id', $ids)
            ->sum('journal_voucher_details.debit');
    ...
    public function OpenDebit($startDate, $endDate, $account_code)
    {
        $ids = ChartOfAccount::where('account_code', 'like', $account_code . '%')->get(['id']);
    ...
    public function OpenCredit($startDate, $endDate, $account_code)
    {
        $ids = ChartOfAccount::where('account_code', 'like', $account_code . '%')->get(['id']);
    ...
    public function indexWithBalance(Request $request)
    {
        ...
        $chartOfAccountData = ChartOfAccount::with(['child', 'level'])
            ->get();
        // return   $chartOfAccountData;

        $chartOfAccount = $chartOfAccountData->map(function ($q) use ($start, $end) {
            //dd($q);


            $q->Balance = $this->Balance($start, $end, $q->account_code);
            $q->TotalCredit = $this->TotalCredit($start, $end, $q->account_code);
            $q->TotalDebit = $this->TotalDebit($start, $end, $q->account_code);
            $q->OpenDebit = $this->OpenDebit($start, $end, $q->account_code);
            $q->OpenCredit = $this->OpenCredit($start, $end, $q->account_code);
            $q->child->map(function ($q) use ($start, $end) {
                $q->Balance = $this->Balance($start, $end, $q->account_code);
                $q->TotalCredit = $this->TotalCredit($start, $end, $q->account_code);
                $q->TotalDebit = $this->TotalDebit($start, $end, $q->account_code);
                $q->OpenDebit = $this->OpenDebit($start, $end, $q->account_code);
        ...
        return $this->apiResponse(new GeneralCollection($chartOfAccount, ChartAccountResource::class));
    }
