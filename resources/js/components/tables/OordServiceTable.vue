  ...
  <div>
    ...
    <VCardText class="AddRow">
      ...
      <VCol cols="6" md="4">
          ...
          <VBtn  @click="$router.push(`Store`)"> {{ $t('Add New Oorder Service') }}</VBtn>

      </VCol>
    </VCardText>
    <!-- 👉 Datatable  -->
    <VDataTable :headers="$i18n.locale === 'en' ? headers_En : headers_Ar" :items="OordServ" :search="search"
      :items-per-page="10">
      <template #item.return_date="{ item }">
        <VLabel>{{ item.props.title.return_date }}</VLabel>
      </template>
      <template #item.update="{ item }">
        <VBtn color="primary" @click="Update(item.raw?item.raw:item)">{{ $t('Update') }}
        </VBtn>
      </template>
      <template #item.actions="{ item }">
        <VBtn v-if="!item.props.title.is_return" color="primary" @click="returnOORD(item)">{{ $t('Return') }}
        </VBtn>
      </template>
      <template #item.is_return="{ item }">
        <VChip class="ma-2">
          <VIcon v-if="item.props.title.is_return" icon="mdi-check" color="success" />
          <VIcon v-if="!item.props.title.is_return" icon="mdi-close-octagon" color="error" />
        </VChip>
      </template>

    </VDataTable>

    <VDialog v-model="updateDialog" width="1024" persistent z-index="1000">
      <VCard>
        <VCardTitle>
            ...
            <VContainer>
              <VRow>
                <VCol cols="12" sm="6" md="4">
                  <AppDateTimePicker v-model="end_date_edit" label="End date"
                    :config="{ altInput: true, altFormat: 'Y-m-d l', dateFormat: 'Y-m-d' }" clearable
                    :rules="[requiredValidator]" />
                </VCol>
                <VCol cols="12" sm="6" md="12">
                  <VTextarea v-model="note_edit" label="Note" clearable :rules="[requiredValidator]" />
                </VCol>
              </VRow>
            </VContainer>
          </VCardText>
