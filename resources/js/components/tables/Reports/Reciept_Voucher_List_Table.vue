    ...
    <VCol cols="12" md="12">
      <VCard class="d-print-none">
        ...
        <div style="display: flex;width: 55%;justify-content: space-between;float: inline-end;">
          ...
          <VCol>
            <AppDateTimePicker v-model="end_dates" :config="{ altInput: true, altFormat: 'Y-m-d l', dateFormat: 'Y-m-d' }"
              :placeholder="$t('end date')" />
          </VCol>
          <VCol>
            <VBtn @click="getReceipt" style="float: inline-end;">
              {{ $t('Search') }}
            </VBtn>
          </VCol>
          <VCol>
          ...
          <VCol cols="12" sm="12" md="12">
            ...
            <VCol style="padding: 2px;" cols="12" sm="12" md="12">
              <table style=" width: 100%;  color: black;font-size: 12px;">
                ...
                <tbody>
                  <tr v-for="(item, itemObjKey) in ReceiptVoucher" :key="itemObjKey"  style="height: 20px; text-align: center">
                    ...
                    <td style="border-bottom: 1px solid;">{{ item.voided_at }}</td>
                    <td style="border-bottom: 1px solid;"></td>
                    <td style="border-bottom: 1px solid;">{{ item.guest.profile.first_name }}</td>
...
export default {
  ...
  methods: {
    ...
    getReceipt() {
      axios.post(`${this.URL}/api/receipt-voucher-list-report`, {
        startDate: this.start_dates,
        endDate: this.end_dates,
      }).then(res => (this.ReceiptVoucher = res.data.receipt_voucher_list, this.dataCount = res.data.receipt_voucher_list.length,
        this.ReceiptVoucher.forEach((ele, value) => this.SumsPayment += ele.payment_amount)))
