      ...
      <VCard class="d-print-none">
        ...
        <div
          ...
          <VCol>
            ...
            <VCombobox v-model="oord_type" :items="oord_types" :label="$t('oord type')" style="width: 160px;"  item-color="customColorValue" >
            ...
            <VCol cols="12" sm="12" md="12">
              ...
              <VCol style="padding: 2px;" cols="12" sm="12" md="12">
                <table style=" width: 100%;  color: black;font-size: 12px;">
                  ...
                  <tbody>
                    <tr v-for="item in Details" class="text-center"  style="height: 20px; text-align: center">
                      <td style="border-bottom: 1px solid;">{{ item.room.rm_name_en }}</td>
                      <td style="border-bottom: 1px solid;">{{ item.room.room_type.rm_type_name }}</td>
                      <td style="border-bottom: 1px solid;">{{ item.start_date }}</td>
                      <td style="border-bottom: 1px solid;">{{ item.end_date }}</td>
                      <td style="border-bottom: 1px solid;"> {{ item.notes }}</td>
                      <td style="border-bottom: 1px solid;">{{ item.room.fo_status }} {{ item.room.hk_stauts }}</td>
                      <td style="border-bottom: 1px solid;">{{ item.return_date }}</td>
                    </tr>
                    <tr>
...
export default {
  ...
  methods: {
    getDetails() {
      axios.post(`${this.URL}/api/OO-and-oS-report`, {
        startDate: this.start_dates,
        endDate: this.end_dates,
        oord_type: this.oord_type,
      }).then(res => (this.Details = res.data.OO_AND_OS, this.dataCount = res.data.OO_AND_OS.length
      ))
