          ...
          <VRow>
            ...
            >
              ...
              <VCol
                ...
                md="12"
              >
                <VTable  v-for="(item, key) in House_Keeping" :key="key">
                  <thead>
                  <tr>
                    <th class="text-center">
                      <h1 style="color: black;">  {{ key }} </h1>
                    </th>
                  </tr>
                  <br><br>
                  </thead>
                  <VCard   v-for="value in item" style="color: black;margin:0% 5px;width: 12%;display: inline-block;text-align: center">
                    <VCardText  style=""><b>{{value}}</b></VCardText></VCard>
                  <tbody>
                  <tr>
                    <td></td>
                  </tr>
                  <tr>
                    <td style="box-shadow: 0px 5px 8px 0px;text-align: center"  v-if="dataCount > 0">
...
export default {
  ...
  methods:{
    getHouse() {
      axios.get(`${this.URL}/api/house-keeping-by-status`)
        .then(res =>(this.House_Keeping = res.data.House_Keeping_By_Status,this.dataCount = res.data.House_Keeping_By_Status.length))
    },
    async printInvoice(){
      await this.getHouse()
