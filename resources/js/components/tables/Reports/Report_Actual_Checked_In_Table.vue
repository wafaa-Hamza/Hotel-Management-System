    ...
    <VCol cols="12" md="12">
      <VCard class="d-print-none">
        ...
        <div style="display: flex;width: 55%;justify-content: space-between;float: inline-end;">
          ...
          <VCol>
            <VBtn style="float: inline-end;" @click="getActual">
              {{ $t('Search') }}
            </VBtn>
          </VCol>
          <VCol>
          ...
          <VCol cols="12" sm="12" md="12">
            ...
            <VCol style="padding: 2px;" cols="12" sm="12" md="12">
              <table style=" width: 100%;  color: black;font-size: 12px;">
                ...
                <tbody>
                  <tr v-for="(item, itemObjKey) in Actual" :key="itemObjKey"  style="height: 25px;">
                    ...
                    <td style="border-bottom: 1px solid;">{{ item.rate_code ? item.rate_code.rate_code : '' }}</td>
...
export default {
  ...
  methods: {
    ...
    getActual() {
      axios.post(`${this.URL}/api/actual-checkedIn-and-arrivals-report`, {
        startDate: this.start_dates,
        endDate: this.end_dates,
      }).then(res => (this.Actual = res.data.Actual_CheckedIn_And_Arrivals_Report, this.dataCount = res.data.Actual_CheckedIn_And_Arrivals_Report.length))
