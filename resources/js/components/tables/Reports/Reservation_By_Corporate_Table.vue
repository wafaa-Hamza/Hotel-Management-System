    ...
    <VCol cols="12" md="12">
      <VCard class="d-print-none">
        ...
        <div style="display: flex;width: 65%;justify-content: space-between;float: inline-end;">
          ...
          <VCol>
            <VCombobox v-model="company_id" :items="company" item-title="company_name" :label="$t('Company')"  item-color="customColorValue"
                       item-value="id">
        ...
        <VRow>
          <VCol cols="12" sm="12" md="12">
            <VCol style="border: 1px solid black;border-radius: 10px;text-align: center;" cols="12" sm="12" md="12">
              ...
              <h2>Reservation By Corporate</h2>
                ...
                <tbody>
                  <tr v-for="item in Details">
                    ...
                    <td style="border-bottom: 1px solid;">
                      <p v-if="item.created_by">{{ item.created_by.firstname }} {{ item.created_by.lastname }}</p>
                    </td>
                    <td style="border-bottom: 1px solid;">{{ item.res_status }}</td>
                    <td style="border-bottom: 1px solid;">{{ item.transactions_sum_charge_amount }}</td>
                  </tr>
                  <tr>
                    ...
                    <td />
                    <td v-if="dataCount > 0" style="box-shadow: 0 5px 8px 0;text-align: center;">
                      <b>Sum => {{ parseFloat(tot_sum_charge).toFixed(2) }}</b>
                    </td>
                    <td></td>
                  </tr>
                </tbody>
...
export default {
  ...
  methods: {
    ...
    GetCompanyProfiles() {
      axiosIns
        .get(`${window.location.origin}/api/companyProfile`).then(res => this.company = res.data)
    },
    getDetails() {
      axios.post(`${this.URL}/api/reservations-by-company`, {
        startDate: this.start_dates,
        endDate: this.end_dates,
        company_id: this.company_id.id,
      }).then(res => (this.Details = res.data.reservation_by_company, this.dataCount = res.data.reservation_by_company.length,
        this.Details.forEach((ele, value) => { this.tot_sum_charge += ele.transactions_sum_charge_amount })))
