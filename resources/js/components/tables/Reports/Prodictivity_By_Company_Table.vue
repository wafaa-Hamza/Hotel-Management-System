        ...
        <VRow>
          <VCol cols="12" sm="12" md="12">
            <VCol style="border: 1px solid black;border-radius: 10px;text-align: center;" cols="12" sm="12" md="12">
              ...
              <h2>Prodictivity By Corporate</h2>
            ...
            <VCol style="padding: 2px;" cols="12" sm="12" md="12">
              <table style=" width: 100%;  color: black;font-size: 12px;">
                ...
                <tbody>
                  <tr v-for="item in Details" :key="item.id"  style="height: 20px;text-align: center">
                    <td style="border-bottom: 1px solid;">{{ item.company.company_name }}</td>
                  ...
                  <tr>
                    ...
                    <td v-if="dataCount > 0" style="box-shadow: 0 5px 8px 0;text-align: center;">
                      <b>Sum => {{ parseFloat(tot_fb).toFixed(2) }}</b>
                    </td>
                    <td></td>
                    <td></td>
                    <td></td>
...
export default {
  ...
  methods: {
    ...
    getDetails() {
      axios.post(`${this.URL}/api/ProductivityByCompany`, {
        start_date: this.start_dates,
        end_date: this.end_dates,
      }).then(res => (this.Details = res.data, this.company = res.data.company, this.dataCount = res.data.length,
        this.Details.forEach((ele, value) => { this.tot_fb += ele.FB, this.tot_rate += ele.Total_Room_Rate })
      ))
