    ...
    <VCol cols="12" md="12">
      <VCard class="d-print-none">
        ...
        <div style="display: flex;width: 55%;justify-content: space-between;float: inline-end;">
          ...
          <VCol>
            <VBtn @click="getRate" style="float: inline-end;">
              {{ $t('Search') }}
            </VBtn>
          </VCol>
          <VCol>
            ...
            <VCol cols="12" sm="12" md="12">
              ...
              <VCol style="padding: 2px;" cols="12" sm="12" md="12">
                <table style=" width: 100%;  color: black;font-size: 12px;">
                  ...
                  <tbody>
                    <tr v-for="(item, itemObjKey) in RateChange" :key="itemObjKey"  style="height: 20px;text-align: center">
                     <td style="border-bottom: 1px solid;">{{ item.hotel_date }}</td>
                     <td style="border-bottom: 1px solid;">{{ item.room?item.room.rm_name_en:'' }}</td>

                      <td v-for="from in item.rate_code_from" style="border-bottom: 1px solid;">{{ from.rate_code }}</td>
                      <td v-for="to in item.rate_code_to" style="border-bottom: 1px solid;">{{ to.rate_code }}</td>
                     <td style="border-bottom: 1px solid;">{{ item.from_rm_rate }}</td>
                     <td style="border-bottom: 1px solid;">{{ item.to_rm_rate }}</td>
                    </tr>
...
export default {
  ...
  methods: {
    getRate() {
      axios.post(`${this.URL}/api/rate_change_report`, {
        startDate: this.start_dates,
        endDate: this.end_dates,
      }).then(res => (this.RateChange = res.data.rate_change_report, this.dataCount = res.data.rate_change_report.length))
